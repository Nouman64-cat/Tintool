...
const ColorCombinationContent: React.FC<ColorCombinationContentProps> = ({
  ...
  return (
    <div>
      {/* Main Heading */}
      <div>
        <h1 className="flex justify-center items-center w-full text-5xl text-gray-900 font-serif mt-10">{heading}</h1>
        <h3 className="flex justify-center items-center text-xl font-semibold text-gray-850 font-serif mt-8">{subheading}</h3>
        <h5 className=" flex justify-center items-center text-lg text-gray-700 font-serif mt-8">
          {subheading_paragraph}
        </h5>
      </div>
      ...
      {[ 
        ...
        <div key={index}>
          <h3 className="font-bold text-gray-800 font-serif mt-8 ">{section.title}</h3>
          <p className="text-lg text-gray-600 font-serif mt-6">{section.content}</p>
        </div>
      ))}
    </div>
  );
